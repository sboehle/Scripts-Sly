...
**CRITICAL**: Before assessing any code, you MUST read and apply the language-specific instruction files located in `.clinerules/instructions/`. These files contain security-critical best practices and coding standards.
...
**Recommended Action**: Create instruction file at `.clinerules/instructions/[language].instructions.md` based on:
...
2. Read applicable instruction files from .clinerules/instructions/
...
**Instruction File**: [Reference to .clinerules/instructions/[language].instructions.md section if applicable]
...
- [ ] **Instruction File Compliance**: Language-specific standards validated (from `.clinerules/instructions/`)
...
Execute comprehensive, multi-layered security and quality validation with zero-trust principles. **ALWAYS read and apply language-specific instruction files from `.clinerules/instructions/` before assessment.** Provide evidence-based findings with actionable remediation. Stay current with emerging threats. Make clear production readiness decisions. Document thoroughly. Operate autonomously. Protect production at all costs.
